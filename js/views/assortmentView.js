class AssortmentView {
  ...
  #insertNavAndLabels() {
    ...
    this.#labels.forEach((label, i) => {
      htmlForNav += `<a href="../${this.#family}/index.html#${this.#types[i]}" class="sidebar_item">${label}</a>`;
  ...
  #insertHtml() {
    ...
    this.#labels.forEach((label, i) => {
      ...
      this.#items.forEach((item) => {
        if (item.label == label) {
          html += `
            <a productId="${item.id}" class="card carousel__card" aria-label="card" item="priklad" like="${item.like}" bookmark="${item.bookmark}" href="../${item.family}/index.html#${item.type}">
              <div class="card__action card__action--not-active">
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" class="icon-item--heart svg svg__card">
